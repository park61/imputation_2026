...
# ── 설정 ──────────────────────────────────────────────────────────────────────
FOLDS_TO_RUN = list(range(1, 11))   # 1~10 전체 (재수행)
...
REGULARIZATION_LAMBDA = 0.0   # lambda=0: 재수행 대상 3개메서드와 동일 조건

METHODS = [
    "msd", "jmsd", "acos",   # 재수행 대상만
]
...
for fold_num in FOLDS_TO_RUN:
    ...
    # 이미 완료된 fold 건너뜀 — 단, 현재 METHODS가 기존 CSV에 모두 있을 때만 skip
    existing = sorted([f for f in os.listdir(res_dir)
                if f.startswith("grid_search_results_") and f.endswith(".csv")])
    if existing:
        latest_csv = os.path.join(res_dir, existing[-1])
        df_ex = pd.read_csv(latest_csv)
        already_in = set(df_ex['method'].unique()) if 'method' in df_ex.columns else set()
        if set(METHODS).issubset(already_in):

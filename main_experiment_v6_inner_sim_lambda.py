# -*- coding: utf-8 -*-
"""
main_experiment_v6_inner_sim_lambda.py
----------------------------------------------------------------------
inner_sim 지정 fold 결과에 대해 여러 lambda 값을 재평가.

전략:
  1. results/inner_sim/fold_XX/alpha_optimization_history_*.csv 로드
  ...
  4. 결과를 results/inner_sim/fold_XX/ 에 lambda 별로 저장
     파일명: grid_search_results_inner_lambda_{lam}_v1_{timestamp}.csv

실행:
    python main_experiment_v6_inner_sim_lambda.py --fold 2
"""
...
_parser.add_argument("--fold", type=int, required=True,
                     help="fold 번호 (예: --fold 2)")
_parser.add_argument("--lambdas", type=float, nargs="+", default=None,
                     help="lambda 값 목록 (예: --lambdas 0.0  또는 --lambdas 0.0 0.001)")
...
# fold_01 분석 결과: 0.0001~0.0003은 0.0005와 결과 거의 동일 → 제거
_DEFAULT_LAMBDAS = [0.0005, 0.001, 0.002, 0.003, 0.005, 0.010, 0.015, 0.020, 0.030]
LAMBDA_VALUES = _args.lambdas if _args.lambdas is not None else _DEFAULT_LAMBDAS
...
# ── 메인 ──────────────────────────────────────────────────────────────────────
print("=" * 70)
print(f"  inner_sim multi-lambda re-evaluation  ({FOLD_ID})")
